...
def load_data(source):
    ...
    if (source == 'bridge'):
        ...
        cheongdam = sorted(
            glob(f"{script_dir}/../data/bridge/서울시(청담대교)/*.csv"))
    ...
    # 강수량 ------------------------------------------
        ...
        data_list = {'zingwan': zingwan,
                     'daegog': daegog, 'songjeong': songjeong}
...
def merge_data(source):
    ...
    for river in data.keys():
        ...
        if source != 'tide':
            full_df['ymdhm'] = pd.to_datetime(
                full_df['ymdhm'], format='%Y%m%d%H%M')
        # 조위 데이터의 경우 형식이 달라 개별 처리
        else:
            full_df['record_time'] = pd.to_datetime(
                full_df['record_time'], format='%Y-%m-%d %H:%M:%S')
...
def finalize_data():
    ...
    if (answer == 1):
        print(check_data())
        if (check_data() == 0):
            dam_data = pd.read_csv(
                f"{script_dir}/../data/dam/full_dam.csv", index_col=0)
            bridge_data = pd.read_csv(
                f"{script_dir}/../data/bridge/full_bridge.csv", index_col=0)
            rf_data = pd.read_csv(
                f"{script_dir}/../data/rf/full_rf.csv", index_col=0)
            tide_data = pd.read_csv(
                f"{script_dir}/../data/tide/full_tide.csv", index_col=0)
            ...
            final_data = pd.read_csv(
                f"{script_dir}/../data/full_data.csv", index_col=0)
    else:
        print("작업을 종료합니다.")
        return 0
    return final_data
